...
pub fn million_program(program: Program, ctx: ProgramStateContext) -> Program {
    let auto_program = match ctx.options.auto {
        Some(true) => program.fold_with(&mut million_auto_program(ctx.clone())),
        _ => program,
    };
    return auto_program.fold_with(&mut million_block_transform(ctx.clone()));
}
